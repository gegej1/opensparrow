...
## 1) Product goal
Build and maintain the portable Feishu delivery chain inside the unified `opensparrow/` source repo.
The package must include one reusable skill, one executable setup script, one hardening script, one SOP, and the required wrapper/UI/doc assets.
It must support a clean machine setup with Feishu WebSocket channel and OpenAI model access while remaining isolated by default.
...
5. Maintainer continues cleanup/containerization planning from the unified repo root.
...
- Runtime: Bash + PowerShell + wrapper templates under `platforms/{mac,windows}/wrappers/`
- Framework: OpenClaw CLI + USB/UI delivery assets
- Package manager: npm
...
- Stable modules that must not break: `scripts/openclaw-usb/*`、`ui/`、`platforms/{mac,windows}/wrappers/` 与现有 runbook/doc 路径
- APIs/contracts that must remain backward compatible: existing OpenClaw CLI commands in docs and install/hardening parameters
- Files/directories that cannot be touched: `.codex/auth.json`, `.codex/config.toml`, frozen legacy directories
