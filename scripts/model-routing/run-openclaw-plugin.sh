...
"$node_bin" "$repo_root/scripts/model-routing/manage-clawrouter-plugin.mjs" install-runtime >/dev/null

eval "$($node_bin "$repo_root/scripts/model-routing/manage-clawrouter-plugin.mjs" openclaw-env | sed 's/^/export /')"
